...
export default function LandingPage() {
  useEffect(() => {
    ...
    // Fix für den Flash-Effekt des interaktiven Rasters
  ...
  return (
      <main>
        <section className="relative flex min-h-screen w-full flex-col items-center justify-center overflow-hidden bg-dark-background p-8">
          <div aria-hidden="true" className="pointer-events-none absolute inset-0 z-0">
            ...
            <div id="interactive-grid" className="absolute inset-0 bg-[linear-gradient(to_right,#9ca3af33_1px,transparent_1px),linear-gradient(to_bottom,#9ca3af33_1px,transparent_1px)] bg-[size:40px_40px] opacity-0 transition-opacity duration-300" style={{ mask: 'radial-gradient(circle 250px at var(--mouse-x) var(--mouse-y), black, transparent)', WebkitMask: 'radial-gradient(circle 250px at var(--mouse-x) var(--mouse-y), black, transparent)' }}></div>
          </div>

          <div className="relative z-10 flex flex-col items-center text-center shadow-indigo-500/50 shadow-2xl">
            <h1 className="text-5xl font-bold tracking-tight text-white md:text-7xl shadow-indigo-500/50 shadow-2xl">
